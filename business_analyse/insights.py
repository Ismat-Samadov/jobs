...
engagement_values = [50000, 10000, 30, 70]  
...
with PdfPages('Job_Vacancy_Platform_Presentation.pdf') as pdf:
    ...
    ax.set_title('Market Opportunity', fontsize=12)
    ...
    plt.savefig('slide_market_opportunity.png')
    pdf.savefig(fig)
    plt.close()
    ...
    ax.set_title('Revenue Model', fontsize=12)
    ...
    plt.savefig('slide_revenue_model.png')
    pdf.savefig(fig)
    plt.close()
    ...
    ax.set_title('Financial Projections', fontsize=12)
    ...
    plt.savefig('slide_financial_projections.png')
    pdf.savefig(fig)
    plt.close()
    ...
    ax1.set_title('Market Trends', fontsize=12)
    ...
    ax2.set_title('User Demographics', fontsize=12)
    
    fig.suptitle('Market Insights', fontsize=12)
    ...
    plt.savefig('slide_market_insights.png')
    pdf.savefig(fig)
    plt.close()
    ...
    ax.set_title('Technology Adoption Rates', fontsize=12)
    ax.set_xlabel('Year', fontsize=12)
    ...
    plt.savefig('slide_tech_adoption.png')
    pdf.savefig(fig)
    plt.close()
    ...
    plt.savefig('slide_user_engagement.png')
    pdf.savefig(fig)
    plt.close()
