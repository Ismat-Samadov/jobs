"""
UNVAN.AZ Scraper - Classifieds Listings

Scrapes classified listings from unvan.az with:
- POST pagination for listing URLs
- HTML parsing for listing data
...
class UnvanAzScraper:
    """Scraper for unvan.az classified listings"""

    BASE_URL = "https://unvan.az"
    HOMELIST_URL = "https://unvan.az/homelist/"
    ...
    async def fetch_listings_page(self, page_num: int) -> Optional[str]:
        """
        Fetch a page of listings using POST

        Args:
            page_num: Page number (starts at 0, increments by 5)

        Returns:
            HTML response string or None
        """
        try:
            # start parameter: 0, 5, 10, 15, etc.
            start = page_num * 5

            params = {'start': start}

            async with self.session.post(self.HOMELIST_URL, params=params) as response:
                if response.status == 200:
                    html = await response.text()
                    return html
                else:
                    print(f"   ✗ Failed to fetch page {page_num + 1} (HTTP {response.status})")
                    return None

        except asyncio.TimeoutError:
            print(f"   ✗ Timeout fetching page {page_num + 1}")
            return None
    ...
    def extract_listing_urls(self, html: str) -> List[str]:
        ...
        try:
            ...
            # Find all listing divs: <div class="index prodbig">
            listing_divs = soup.find_all('div', class_='index')

            for div in listing_divs:
                # Find link in holderimg or prodname
    ...
    async def extract_phone_params(self, listing_url: str) -> Optional[Dict]:
        ...
        try:
            async with self.session.get(listing_url) as response:
                ...
                # <div id="telshow" data-id="1125846" data-t="elanlar"
                #      data-h="150f6285b569c27ac527983dd6941d96" data-rf="">
                ...
                data_t = telshow_div.get('data-t', 'elanlar')
                ...
                # Extract title
                title = None
                prodname = soup.find('div', class_='prodname')
                if prodname:
                    title_link = prodname.find('b')
                    if title_link:
        ...
        try:
            ...
            async with self.session.post(self.AJAX_URL, data=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    # Response: {"ok": 1, "tel": "0512334569", "msg": "..."}
                    if data.get('ok') == 1:
                        tel_string = data.get('tel', '')

                        if tel_string:
                            # Clean phone number (remove formatting)
                            digits_only = re.sub(r'\D', '', tel_string)
                            if digits_only:
                                return [digits_only]
    ...
    def save_lead(self, phone_number: str, listing_data: Dict) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s)
            """, (phone_number, 'unvan.az', listing_data.get('url'), psycopg2.extras.Json(full_data)))
    ...
    async def scrape(self, max_pages: int = 5):
        ...
        # Print final stats
        print(f"\n{'='*70}")
        print("Scraping Complete - UNVAN.AZ")
        print(f"{'='*70}")
...
async def main():
    ...
    # Run scraper
    async with UnvanAzScraper(db_pool) as scraper:
