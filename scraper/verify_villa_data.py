"""
Verify that Villa.AZ full_data is correctly populated in the database
"""
import psycopg2
...
# Get the latest Villa.AZ lead with full_data
cur.execute("""
...
if result:
    ...
    print("=" * 70)
    print("LATEST VILLA.AZ LEAD WITH FULL_DATA")
    print("=" * 70)
    print(f"Phone: {phone}")
    ...
    print(f"  - features: {len(full_data.get('features', []))} features - {full_data.get('features', [])}")
    print(f"  - address: {full_data.get('address', 'MISSING')[:80]}...")
else:
    print("No Villa.AZ leads with full_data found!")
