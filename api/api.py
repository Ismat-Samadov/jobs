...
async def get_data(
    ...
    if company is None and position is None:
        ...
        query = "SELECT * FROM vacancy_table ORDER BY scrape_date DESC OFFSET $1 LIMIT $2;"
        ...
        query = "SELECT * FROM vacancy_table WHERE company ILIKE $1 AND vacancy ILIKE $2 ORDER BY scrape_date DESC OFFSET $3 LIMIT $4;"
        offset = (page - 1) * page_size
        ...
        query = "SELECT * FROM vacancy_table WHERE company ILIKE $1 ORDER BY scrape_date DESC OFFSET $2 LIMIT $3;"
        offset = (page - 1) * page_size
        rows = await db.fetch(query, f"%{company}%", offset, page_size)
    else:
        # If only position is provided, search by position
        query = "SELECT * FROM vacancy_table WHERE vacancy ILIKE $1 ORDER BY scrape_date DESC OFFSET $2 LIMIT $3;"
        offset = (page - 1) * page_size
