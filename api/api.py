...
async def get_data_by_company(company: str, db=Depends(connect_to_postgres)):
    query = "SELECT * FROM jobs WHERE scrape_date = (select max(scrape_date) from jobs ) and  company ILIKE $1 ORDER BY scrape_date DESC;"
...
async def get_data_by_position(position: str = Query(..., description="Position name to search for"), db=Depends(connect_to_postgres)):
    query = "SELECT * FROM jobs WHERE scrape_date = (select max(scrape_date) from jobs ) and vacancy ILIKE $1 ORDER BY scrape_date DESC;"
...
async def get_data(
    ...
    if company is None and position is None:
        ...
        query = "SELECT * FROM jobs where scrape_date = (select max(scrape_date) from jobs ) ORDER BY scrape_date DESC OFFSET $1 LIMIT $2;"
        ...
        query = "SELECT * FROM jobs WHERE scrape_date = (select max(scrape_date) from jobs ) and company ILIKE $1 AND vacancy ILIKE $2 ORDER BY scrape_date DESC OFFSET $3 LIMIT $4;"
        offset = (page - 1) * page_size
        ...
        query = "SELECT * FROM jobs WHERE scrape_date = (select max(scrape_date) from jobs ) and company ILIKE $1 ORDER BY scrape_date DESC OFFSET $2 LIMIT $3;"
        offset = (page - 1) * page_size
        rows = await db.fetch(query, f"%{company}%", offset, page_size)
    else:
        # If only position is provided, search by position
        query = "SELECT * FROM jobs WHERE scrape_date = (select max(scrape_date) from jobs ) and vacancy ILIKE $1 ORDER BY scrape_date DESC OFFSET $2 LIMIT $3;"
        offset = (page - 1) * page_size
